		...
		<table id="table" class="table responsive-table table-bordered table-striped">
			...
			<tbody>
				...
				$to = date('Y-m-d', strtotime($date['2']));
					...
					// $and = "AND journal_details.request_by = ".$_GET['id'];
				...
				$journal_no = $_GET['id'];
					$entriestable = $connection -> myQuery("SELECT 
						...
						where journal_details.date_of_entry between '$from' and '$to' AND journal_details.request_by = {$_GET['id']}
					...
					journal_details.is_debit
						...
						-- where journal_entries.journal_id=$journal_no 
						 where journal_details.request_by = {$_GET['id']}
						");
